...
export default function DatabaseCenterView({ setView, session }) {
  ...
  const [selectedId, setSelectedId] = useState('sqlite_demo');
  const [expandedId, setExpandedId] = useState('sqlite_demo');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (selectedId === deleteTarget.id) {
        setSelectedId(remaining[0]?.id || 'sqlite_demo');
        setExpandedId(remaining[0]?.id || 'sqlite_demo');
      }
        ...
        <main className="min-w-0 overflow-y-auto">
          {selected && <>
            ...
            <section className="sticky top-0 z-10 px-4 sm:px-7 py-3.5 border-b border-slate-200 bg-white/95 backdrop-blur-md flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2"><button onClick={() => handleTest(selected)} disabled={testingId === selected.id} className="h-9 px-3 rounded-lg border border-slate-200 bg-white text-xs font-bold text-slate-700 inline-flex items-center gap-1.5 disabled:opacity-50">{testingId === selected.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />}Test source</button>{!['sqlite_demo', 'postgres_demo'].includes(selected.id) && <button onClick={() => setDeleteTarget(selected)} className="w-9 h-9 rounded-lg border border-slate-200 bg-white text-slate-400 hover:text-red-600 hover:border-red-200 flex items-center justify-center" title="Delete data source"><Trash2 className="w-4 h-4" /></button>}</div>
            </section>
            ...
            <div className="px-4 sm:px-7 py-6 max-w-[1600px] mx-auto">
              ...
              {!catalog ? <div className="h-80 flex items-center justify-center"><Loader2 className="w-5 h-5 animate-spin text-indigo-500" /></div> : <>
                ...
                {activeSection === 'relationships' && <section><div className="pb-5"><h2 className="text-sm font-bold text-slate-900">Relationship map</h2><p className="text-[11px] text-slate-500 mt-1">Hover a table to isolate its foreign-key paths. Bridge tables are identified automatically.</p></div><ERDiagram catalog={catalog} /></section>}
